...
uses
  System.Classes, System.SysUtils, System.RegularExpressions, System.Character;
...
begin
  ...
  begin
    if c.IsLower then
      charType := charType or 1
    else if c.IsUpper then
      charType := charType or 2
    else if c.IsDigit then
      charType := charType or 4
    else if (c <= '/') or
       ((':' <= c) and (c <= '@')) or
       (('[' <= c) and (c <= '`')) or
...
function CrackTimeToScore(ACrackTimeSeconds: Double): Integer;
begin
  if (ACrackTimeSeconds < Power(10, 2)) then
    Result := 0
  else if (ACrackTimeSeconds < Power(10, 4)) then
    Result := 1
  else if (ACrackTimeSeconds < Power(10, 6)) then
    Result := 2
  else if (ACrackTimeSeconds < Power(10, 8)) then
    Result := 3
  else
    Result := 4;
end;
